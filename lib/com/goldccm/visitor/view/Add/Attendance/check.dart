...
import 'package:visitor/com/goldccm/visitor/view/Add/Attendance/statistical.dart';
...
//      ],controller: _tabController,),
  ...
  void initState() {
    ...
    AMapLocationClient.startup(new AMapLocationOption(
        desiredAccuracy: CLLocationAccuracy.kCLLocationAccuracyHundredMeters,allowsBackgroundLocationUpdates: false));
...
//      AMapLocationClient.startLocation();
      ...
      setState(() {
        _location = loc;
        print(_location.code);
        if(_location.code==12){
          shownAddress="无法定位,请打开系统的定位服务";
        }else{
          shownAddress = getLocationStr(loc);
        }
      });
...
//                            ),
   ...
   */
   checkNormal() async {
     await initVariables();
    if(_location.code!=0){
      ToastUtil.showShortClearToast("定位失败");
      return ;
    }
    await getPhoto();
