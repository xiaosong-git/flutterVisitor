...
//                               color: Colors.green,
  ...
  bool checkLoginStatus(){
    ...
    if(RegExpUtil().verifyPhone(_loginName)){
      return true;
    }
    if(RegExpUtil().verifyCode(_checkCode)){
      return true;
    }
    return false;
  }
  ...
  *  用户名校验
  ...
  bool checkLoginUser() {
    ...
    if (_loginName == null || _loginName == "") {
      ...
      checkResult = false;
    } else if (!RegExpUtil().verifyPhone(_loginName)) {
      ToastUtil.showShortClearToast('手机号格式不正确');
      checkResult = false;
    } else {
      checkResult = true;
    }
  ...
  bool checkPass() {
    ...
    if (_pass == null || _pass == "") {
      ...
      checkResult = false;
    } else if (_pass.length < 6 && _pass.length >= 32) {
  ...
  Future _loginAction() async {
    bool userNameCheck = checkLoginUser();
    String _passNum;
    String _codeNum;
    var data;
